...
### 2. システムのデプロイ
まず、Mac 側のターミナルで以下のコマンドを実行し、ファイルをラズパイに転送します。
```bash
# Macのターミナルから実行 (100.116.116.96 の部分は自分のラズパイのIPに合わせて書き換えてください)
scp -r ~/Desktop/synage pi@100.116.116.96:~/
```
※ **注意**: IPアドレス（100.116.116.96）は環境によって変わるため、ラズパイの実際の IP を入力してください。

次に、ラズパイに SSH でログインし、以下の2行を順番に実行してセットアップを完了させます。
```bash
# まず、ファイルがあるフォルダへ移動する
cd ~/synage/

# セットアップスクリプトを「実行」する。これが環境構築の本体です。
bash setup.sh
```
- これにより、Nginx、Python サーバー、同期マネージャーがインストール・サービス化 (Systemd) されます。
- **電源を入れるだけで自動的にサイネージが起動**するようになります。
...
### 💡 運用のTips：アーカイブ用フォルダ
- Google Drive の同期フォルダ内に **「アーカイブ」などの名前でサブフォルダ** を作っても、その中身は無視されます。
- 表示し終わった古い掲示物などをそのフォルダへ移動させることで、削除せずに「非表示」にする運用が可能です。
